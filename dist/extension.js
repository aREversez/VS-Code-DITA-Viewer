...
<pre>${yt(m)}</pre>
    ...
    ${u?`<ul class="map-tree">${u}</ul>`:""}
  </li>`}var kt={"map/map":(e,n,i)=>{let s=e.children.find(u=>u.type==="element"&&u.baseType==="map/map-title"),c=s?`<h1 class="map-title">${de(s)}</h1>`:"",p=e.children.filter(u=>u.type!=="element"||u.baseType!=="map/map-title").filter(u=>u.type==="element").map(u=>i(u,n)).join("");return`<div class="ditamap-container">
      ${c}
      <ul class="map-tree">${p}</ul>
    </div>`},"map/map-title":(e,n,i)=>`<h1 class="map-title">${ee(de(e))}</h1>`,"map/topicref":Ge,"map/topichead":(e,n,i)=>{let s=qe(e),c=J(e,n,i);return`<li class="map-tree-item map-tree-item--head">
      <span class="map-tree-label map-tree-label--head">${ee(s)}</span>
      ${c?`<ul class="map-tree">${c}</ul>`:""}
    </li>`},"map/topicgroup":(e,n,i)=>`<li class="map-tree-item map-tree-item--group">
      <ul class="map-tree">${J(e,n,i)}</ul>
...
</html>`}}};function Ft(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Ot(e){e.subscriptions.push(q.window.registerCustomEditorProvider("ditaViewer.preview",new le(e),{webviewOptions:{retainContextWhenHidden:!0}})),e.subscriptions.push(q.window.registerCustomEditorProvider("ditaViewer.mapPreview",new pe(e),{webviewOptions:{retainContextWhenHidden:!0}}));let n=q.commands.registerCommand("ditaViewer.showRendered",()=>{let i=q.window.activeTextEditor;i&&q.commands.executeCommand("vscode.openWith",i.document.uri,"ditaViewer.preview",q.ViewColumn.Beside)});e.subscriptions.push(n)}0&&(module.exports={activate});
