...
</div>`}function et(e){return`<p class="book-skip">(Skipped: ${ce(e)} already included above)</p>`}function tt(e){let{filePath:t,keyMap:i,asWebviewUri:r,headingLevel:s}=e;try{if(!(0,oe.existsSync)(t))return{html:"",error:`File not found: ${t}`};let d=(0,oe.readFileSync)(t,"utf-8"),p=Z(d),u=ae(p),f=Se(u.root),m=Ht(u.root),g=(0,he.dirname)(t),w=_e(g),v=ke(g),T=y=>{let S=f.get(y);if(S)return S;if(y.includes("#"))return v(y)},b=ge(u.root,{headingLevel:s,asWebviewUri:r,documentDir:g,resolveTitle:T,resolveKey:y=>i.get(y),resolveConref:y=>w(y),noteLabels:m}),A=(u.root.children||[]).find(y=>y.type==="element"&&y.baseType==="topic/title"),D=A?ve(A):void 0;return{html:b,title:D}}catch(d){let p=d instanceof Error?d.message:String(d);return{html:"",error:`Error rendering ${t}: ${p}`}}}function qt(){return`
(function() {
...
</html>`}}};function Gt(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function nt(e){return e.replace(/<\/script>/gi,"<\\/script>")}function $e(e){let t=[],i=(0,C.dirname)(e.fsPath),r=Re(i),s=i;for(;s.length>=r.length;){try{for(let p of(0,O.readdirSync)(s))p.endsWith(".ditamap")&&t.push((0,C.join)(s,p))}catch{}if(t.length>0)return t;let d=(0,C.dirname)(s);if(d===s)break;s=d}return t}function ot(e){return e.type==="text"?e.text||"":(e.children||[]).map(ot).join("")}function Yt(e,t){for(let i of t){let r=(e.children||[]).find(s=>s.type==="element"&&s.baseType===i);if(r){let s=ot(r).trim();if(s)return s}}}function jt(e){let t=(e.children||[]).find(i=>i.type==="element"&&i.baseType==="map/topicmeta");if(t)return Yt(t,["map/keyword","map/linktext","map/navtitle","map/shortdesc"])}function Ie(e){let t=new Map,i=$e(e);for(let s of i)try{let f=function(m){if(m.type!=="element")return;let g=m.baseType;if((g==="map/topicref"||g==="map/keydef")&&m.attributes?.keys){let w=m.attributes.keys,v=jt(m);t.set(w,v||w)}for(let w of m.children||[])f(w)};var r=f;let d=(0,O.readFileSync)(s,"utf-8"),u=me(Z(d)).root;for(let m of u.children||[])f(m)}catch{}return t}function Xt(e){let t={},i=new Set,r=m=>{let g=(0,C.basename)(m);if(!i.has(g)&&(0,O.existsSync)(m))try{t[g]=(0,O.readFileSync)(m,"utf-8"),i.add(g)}catch{}},s=(0,C.dirname)(e.fsPath),d=Re(s),p=Kt(s),u=new Set;u.add(p),d!==p&&u.add(d);try{let g=E.workspace.getConfiguration("dita-viewer").get("cssDirectory");if(g)for(let w of g){let v=Zt(w,s);v&&(0,O.existsSync)(v)&&!u.has(v)&&u.add(v)}}catch{}for(let m of u)try{for(let g of(0,O.readdirSync)(m))g.toLowerCase().endsWith(".css")&&r((0,C.join)(m,g))}catch{}try{let g=E.workspace.getConfiguration("dita-viewer").get("customCss");if(g)for(let w of g){let v=Qt(w,s);v&&r(v)}}catch{}let f=t["custom.css"]?"custom.css":Object.keys(t)[0]||"";return{files:t,defaultName:f}}function Kt(e){let t=Re(e),i=e;for(;i.length>=t.length;){if((0,O.existsSync)((0,C.join)(i,"custom.css")))return i;let r=(0,C.dirname)(i);if(r===i)break;i=r}return e}function Re(e){let t=E.workspace.workspaceFolders;if(t&&t.length>0)return t[0].uri.fsPath;let i=e.includes("/")?"/":"\\",r=e.split(/[\\/]/);return i==="/"?"/"+r.slice(1,2).join("/"):r.length>2?r.slice(0,2).join("\\"):e}function Qt(e,t){if((0,C.isAbsolute)(e)&&(0,O.existsSync)(e))return e;let i=(0,C.resolve)(t,e);if((0,O.existsSync)(i))return i;let r=E.workspace.workspaceFolders;if(r)for(let s of r){let d=(0,C.resolve)(s.uri.fsPath,e);if((0,O.existsSync)(d))return d}}function Zt(e,t){if((0,C.isAbsolute)(e))return(0,O.existsSync)(e)?e:void 0;let i=(0,C.resolve)(t,e);if((0,O.existsSync)(i))return i;let r=E.workspace.workspaceFolders;if(r)for(let s of r){let d=(0,C.resolve)(s.uri.fsPath,e);if((0,O.existsSync)(d))return d}}var B=ue(require("vscode"));function ne(e){return e.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function rt(e,t){return t==null?"":` ${e}="${ne(t)}"`}function ie(e,t){return e.attributes?.[t]}function ye(e){return e.type==="text"?e.text||"":(e.children||[]).map(ye).join("")}function st(e,t){for(let i of t){let r=(e.children||[]).find(s=>s.type==="element"&&s.baseType===i);if(r){let s=ye(r).trim();if(s)return s}}}function Le(e){let t=ie(e,"keys"),i=ie(e,"href"),r=(e.children||[]).find(s=>s.type==="element"&&s.baseType==="map/topicmeta");if(r){let s=st(r,["map/navtitle","map/linktext","map/shortdesc"]);if(s)return s;let d=r.children.find(p=>p.type==="element"&&p.baseType==="map/keywords");if(d){let p=st(d,["map/keyword"]);if(p)return p}}if(i){let s=i.replace(/\\/g,"/").split("/"),d=s[s.length-1]||"",p=d.lastIndexOf(".");return p>0?d.substring(0,p):d}return t||"(unnamed)"}function Jt(e){return!!ie(e,"href")}function le(e,t,i){return(e.children||[]).filter(r=>r.type==="element").map(r=>i(r,t)).join("")}function at(e,t,i){let r=ie(e,"href")||"",s=ie(e,"keys")||"",d=Le(e),p=Jt(e),u=le(e,t,i),f=p?'<span class="map-tree-icon map-tree-icon--file">\u{1F4C4}</span>':'<span class="map-tree-icon map-tree-icon--key">\u{1F511}</span>',m=ne(d),g=rt("data-keys",s),w=r?rt("data-href",r):"",T=!!s&&!r&&s?`<span class="map-tree-keyname">${ne(s)}</span><span class="map-tree-keysep"> \u2192 </span>${m}`:m;return p?`<li class="map-tree-item map-tree-item--nav"${g}${w}>
      <a href="#" class="map-tree-link" data-href="${ne(r)}">${f}<span class="map-tree-label">${T}</span></a>
      ${u?`<ul class="map-tree">${u}</ul>`:""}
    </li>`:`<li class="map-tree-item map-tree-item--keydef"${g}${w}>
    ${f}<span class="map-tree-label map-tree-label--keydef">${T}</span>
    ${u?`<ul class="map-tree">${u}</ul>`:""}
  </li>`}var en={"map/map":(e,t,i)=>{let r=e.children.find(u=>u.type==="element"&&u.baseType==="map/map-title"),s=r?`<h1 class="map-title">${ne(ye(r))}</h1>`:"",p=e.children.filter(u=>u.type!=="element"||u.baseType!=="map/map-title").filter(u=>u.type==="element").map(u=>i(u,t)).join("");return`<div class="ditamap-container">
      ${s}
      <ul class="map-tree">${p}</ul>
    </div>`},"map/map-title":(e,t,i)=>`<h1 class="map-title">${ne(ye(e))}</h1>`,"map/topicref":at,"map/topichead":(e,t,i)=>{let r=Le(e),s=le(e,t,i);return`<li class="map-tree-item map-tree-item--head">
      <span class="map-tree-label map-tree-label--head">${ne(r)}</span>
...
`}var Ee=class{constructor(t){this.context=t}async resolveCustomTextEditor(t,i,r){let s=B.Uri.file((0,q.dirname)(t.uri.fsPath)),d="tree";i.webview.options={enableScripts:!0,localResourceRoots:[B.Uri.file(this.context.extensionPath),s,...(B.workspace.workspaceFolders||[]).map(f=>f.uri)]},i.webview.onDidReceiveMessage(f=>{if(f.type==="refresh")u();else if(f.type==="openTopic"){let m=f.href;if(!m)return;let g=(0,q.dirname)(t.uri.fsPath),w=(0,q.resolve)(g,m),v=B.Uri.file(w);B.commands.executeCommand("vscode.openWith",v,"ditaViewer.preview")}else f.type==="switchMode"&&(d=f.mode,u())});let p=B.workspace.onDidChangeTextDocument(f=>{f.document.uri.toString()===t.uri.toString()&&u()}),u=()=>{let f=this.generateHtml(t,i.webview,d);i.webview.html=f};u(),i.onDidDispose(()=>{p.dispose()})}generateHtml(t,i,r){let s=i.asWebviewUri(B.Uri.file((0,q.join)(this.context.extensionPath,"media","styles.css"))),d=(0,q.dirname)(t.uri.fsPath);try{let p=t.getText(),u=Z(p),f=me(u),m;r==="book"?m=this.renderBookContent(f.root,t,i,d):m=lt(f.root,{docDir:d});let g=tn(r),w=(0,dt.randomBytes)(16).toString("base64"),v=B.window.activeColorTheme;return`<!DOCTYPE html>
...
</html>`}}renderBookContent(t,i,r,s){let d=ct(t),p=Ie(i.uri),u=new Set,f=[];for(let m of d)if(m.href){let g=(0,q.resolve)(s,m.href);if(u.has(g)){f.push(et(m.href));continue}u.add(g);let w=(0,q.dirname)(g),v=A=>{try{let D=(0,q.resolve)(w,A),y=B.Uri.file(D),S=r.asWebviewUri(y);if(S)return S.toString()}catch{}try{let D=(0,q.resolve)(w,A);if((0,we.existsSync)(D)){let y=(0,we.readFileSync)(D),S=A.toLowerCase().split(".").pop()||"";return`data:${S==="png"?"image/png":S==="jpg"||S==="jpeg"?"image/jpeg":S==="gif"?"image/gif":S==="svg"?"image/svg+xml":S==="webp"?"image/webp":"image/png"};base64,${y.toString("base64")}`}}catch{}return""},T=Math.min(1+m.depth,6),b=tt({filePath:g,keyMap:p,asWebviewUri:v,headingLevel:T});b.error?f.push(Je(m.displayName,b.error,m.depth)):f.push(`<div class="book-entry">${b.html}</div>`)}else f.push(Ze(m.displayName,m.depth));return`<div class="ditamap-book">${f.join(`
