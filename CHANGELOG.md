...
- **With a template header, the toolbar no longer covers it.** The toolbar now sits inside the header (right end, wrapping under the brand in a narrow window) instead of floating over it. The previous/next-topic buttons use centered SVG chevrons, and the Template dropdown centers its value like the page-width one.
- **Built-in templates: no gap above the sidebar search box, readable type chips on the active row, tighter Reader spacing.** Aurora's sidebar no longer shows scrolled text above the sticky search box; the topic-type chip on the active row is an outline in the row's own colour in light and dark; Reader's headings, paragraphs and lists sit closer together and its header keeps the brand centered under the toolbar.
