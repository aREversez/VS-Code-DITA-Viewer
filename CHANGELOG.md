...
- **The Explorer's DITA Map tree is laid out like Oxygen's DITA Maps Manager.** The main map itself is the root row — its own `<title>`/`<mainbooktitle>` with keyrefs resolved (a version number in the title shows the key's value, not a dropped word), the map file one click away — instead of the tree starting bare at the map's first children. Submaps (`mapref`, or any reference to a `.ditamap`) are named by the referenced map's own title rather than its file name, including a duplicate reference whose children were never spliced in. Rows carry type icons: a filled map for the main map, a map for submaps, a key for keydefs, a folder for groupings (`topichead`, `frontmatter`/`backmatter` and the book lists), and per information type for topics — checklist (task), lightbulb (concept), references (reference), wrench (troubleshooting), book (glossary) — sniffed from the referenced file's root element.
- **Expand All / Collapse All act on the selected node, sit next to each other, and the state persists.** The two buttons are adjacent in the view's title bar (the native collapse-all button, which sat apart and could only collapse the whole tree, is gone), and both also appear on every row's context menu. Invoked on a selected submap they expand or fold exactly that submap — a ditamap is too large for whole-tree expansion to be the useful default — with nothing selected they act on the main map. Collapse All folds the invoked row together with everything below it, like the same command in any other tree (a chapter `topichead` whose children are all leaf topics has nothing below it to fold, so the descendants-only version of it marked nothing at all and read as dead on exactly the rows it is tried on most); the main map's own row stays open, so collapsing from it lands on the one-level outline a fresh tree starts at. Every branch below the root now starts collapsed, which let `frontmatter` and `keydef` entries come back: each costs one collapsed line until you open it. What you expand survives map edits and reloads (rows are matched by structural id, and the recorded state is re-applied on re-render) and VS Code restarts (recorded per map in workspace state). Expanding no longer scrolls: the old implementation revealed each branch in turn and slid the view to the end of the map, while the new one re-anchors on the node the command acted on and restores the selection there.
- **Templates for Docsite and Book view.** A **Template** dropdown in the toolbar restyles a map's sidebar and page and can add a header (logo, title, tagline, banner image, links) and a footer around it; the choice is remembered per map, separately for Docsite and Book view. Three templates ship built in — **Classic Docs**, **Aurora** (banner header) and **Reader** (serif, book-like) — each with a light and a dark palette that follows VS Code's theme. Add your own by listing folders in the new `dita-viewer.templatesDirectory` setting: each sub-folder is one template, described by a `template.json` (or a `<publishing-template>` `.opt` file for name, tags, preview image and css list) plus its css and a `resources/` folder. The header and footer are data in `template.json`, not HTML, so a template cannot inject markup or scripts. A template's css must be written for this extension's own page structure (`.site-nav`, `#dita-content-root`, …), not for an Oxygen WebHelp output.
...
- Saving a DITA file no longer makes every open preview in the workspace re-render: a preview refreshes only for files it actually used (plus files created or deleted, and images and stylesheets), which matters in workspaces with several maps or a large repository.
...
- **Exported site (DITA-OT HTML): no more white flash when switching pages in dark mode.** With dark mode on, each topic page briefly painted DITA-OT's light default before the enhancements script (`dita-viewer-chrome.js`, which ran at the end of `<body>`) switched it dark — a glaring white flash on every navigation. The theme choice (stored preference, else the OS `prefers-color-scheme`) is now applied to `<html>` by a tiny inline script in `<head>` before any body content paints, `dita-viewer-dark.css` also sets `color-scheme: dark` and a dark document canvas, and the end-of-body script just reads that class for its toggle button.
- **DITA-OT HTML5 export: the top page's topic links now work when the map sits in a sub-folder.** DITA-OT writes the map's landing `index.html` to the site root but computes its link targets relative to the map's own folder, so a map under `maps/` produced `href="../topics/foo.html"` — one level *above* the exported site — and clicking any topic from `index.html` went nowhere (the topic pages themselves were fine). The post-transform step now strips those stray `../` prefixes on `index.html`, turning them back into root-relative `topics/foo.html` links; absolute URLs, `mailto:` and `#anchor` references are left untouched.
- **Switching a map's view no longer blanks its toolbar.** The mode button now swaps Outline, Book and Docsite view *in place*: the preview keeps its document instead of reloading, so the toolbar — and the Mode button parked in its fixed spot at the right — stays on screen the whole time rather than flashing out and back. Book view's loading placeholder no longer interrupts the switch, and each mode's extra controls (sidebar, template, page navigation) appear and disappear by visibility only, so the bar never reflows under the mouse.
- **In every map view the toolbar is a top bar.** It is the first row of the page, above any template header, so it never covers the header or the content (in Docsite and Book view the content scrolls beneath it, not under it; in Outline view the bar is the page's own first row and the outline scrolls in the area below it, so the vertical scrollbar starts under the bar instead of running beside a fixed one and nudging the right-hand buttons left). While the bar is wide enough and no template header already shows a title, its left side shows the map's title (Outline view's own heading already does); on a narrow bar the title hides and the buttons wrap. The **Template** dropdown now sits left of the **Mode** button, and **Tags**, **Flags** and **Filter** follow it, so switching views no longer moves the Mode button away from the mouse. The previous/next-topic buttons use centered SVG chevrons, and the Template dropdown centers its value like the page-width one.
- **Built-in templates: no gap above the sidebar search box, readable type chips on the active row, tighter Reader spacing.** Aurora's sidebar no longer shows scrolled text above the sticky search box; the topic-type chip on the active row is an outline in the row's own colour in light and dark; headings in all three templates use a tighter line height, and Reader's headings, paragraphs and lists sit closer together.
- **Previous/next-topic buttons are the same width as back/forward.** A stylesheet minimum width was overriding the toolbar's shared button width, so the two were wider than the history buttons.
- **A map title given by a key reference shows up.** A `<title>` or `<mainbooktitle>` that is a `keyref` (or contains a `<ph keyref>`) used to come out empty and fall back to the file name, both in a template's header and in the top-left title of the default look; it now shows the key's value, like the outline heading does.
- **The sidebar search box is centered in its own block.** Its padding above and below the input is now equal; the input used to sit low.
