...
- **Profiling/conditional-processing attributes are now highlighted** — content carrying `props`, `platform`, `product`, `audience`, `otherprops`, `base`, `importance`, `rev`, or `status` (the full DITA 1.3 `select-atts` group) now renders inside a tinted highlight box with a small label naming the attribute and value that matched (e.g. `Audience [expert]`), matching Oxygen's default "show profiling" style. This *highlights* profiled content rather than hiding it, by design — the point is knowing what's flagged and for which build, not filtering yet; an actual hide/show filter (driven by a `.ditaval`-style rule set) is a natural follow-on but a separate, larger feature. A new toolbar toggle ("Flags", on by default) hides the highlighting without a re-render, for documents where it gets visually busy. A `test-dita-file/topics/profiling_test.dita` fixture was added, mirroring the reported example, for visually confirming this.
