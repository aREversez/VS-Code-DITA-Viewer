...
- **Templates for Docsite and Book view.** A **Template** dropdown in the toolbar restyles the sidebar and page of a map; the choice is remembered per map, separately for Docsite and Book view. One template ships built in (**Classic Docs**). Add your own by listing folders in the new `dita-viewer.templatesDirectory` setting: each sub-folder is one template, described by a `template.json` or by a `<publishing-template>` `.opt` file (name, colour tags, preview image and css list are read; `webhelp.*` parameters are ignored) plus its css. A template's css must be written for this extension's own page structure (`.site-nav`, `#dita-content-root`, …), not for an Oxygen WebHelp output.
