...
- **Fixed the preview visibly jumping to the top and animating back down on every pause while typing** — a separate, purely visual issue from the feedback-loop fix above. Every source edit re-renders the preview by reassigning `webview.html` wholesale, which is a full page reload and resets scroll to 0; the extension then separately told the reloaded page to smooth-scroll back down to where it had been, 200ms later. On nearly every pause while typing (the render debounce fires on each one), that showed as the preview visibly snapping to the top and animating back — the reported back-and-forth jump. It could also nudge the source editor's own viewport slightly (not the cursor — typing position itself was never affected), if the preview's scroll-sync suppression window happened to still be settling when the animation ended. The current source line is now baked directly into the freshly generated page (a small inline script sets it before the page's own script runs) and applied with an instant, non-animated jump immediately on load, instead of a second round-trip message asking an already-loaded page to animate there afterward — eliminating both the visible reset and the delay.
