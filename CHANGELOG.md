...
- **In Docsite and Book view the toolbar is a top bar.** It is the first row of the page, above any template header, so it never covers the header or the content (the content scrolls beneath it, not under it). While the bar is wide enough and no template header already shows a title, its left side shows the map's title; on a narrow bar the title hides and the buttons wrap. Outline view keeps the floating toolbar. The previous/next-topic buttons use centered SVG chevrons, and the Template dropdown centers its value like the page-width one.
